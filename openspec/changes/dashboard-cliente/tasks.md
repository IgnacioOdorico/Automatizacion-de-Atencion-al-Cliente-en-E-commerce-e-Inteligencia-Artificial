...
- [ ] 9.1 Passwords: bcrypt, nunca texto plano ni comparación en claro (AC: `password_hash` en BD es hash bcrypt; `grep` al repo no encuentra contraseñas planas de la demo) — **sin tildar (cumple con salvedad)**: la BD y `seed_dashboard.sql` guardan solo el hash bcrypt (`$2b$12$`, 60 caracteres), pero la contraseña de la cuenta demo (`Demo2026!`) figura en claro en `dashboard-web/README.md` y en los tests, a propósito, para poder loguearse en la demo; el segundo criterio no se cumple al pie de la letra (evidencia en el ODD)
- [x] 9.2 JWT: expiración corta (2h) + refresh, secreto en `.env` nunca hardcodeado (AC: `.env` y `dashboard-api/.env.example` documentan `DASHBOARD_JWT_SECRET`; no hay secretos en git) — access 7200 s y refresh 604800 s observados; la API no arranca con secretos vacíos, cortos o de ejemplo; refresh como access, `alg=none` y refresh reutilizado dan 401
- [x] 9.3 `encrypted_credentials` encriptado con Fernet y clave `DASHBOARD_ENC_KEY` de `.env` (AC: un SELECT a `channel_connections` muestra criptograma, no el refresh token) — verificado con test de integración contra la BD de pruebas y Google simulado (`test_isolation.py`); el circuito real con Google no se pudo verificar (sin credenciales)
- [x] 9.4 CORS: whitelist explícita (`http://localhost:5173`, `http://localhost:8080`), nunca `*` (AC: request desde otro origen responde el header correcto o falla) — preflight con curl: orígenes permitidos 200, `evil`, `null` y `localhost:8080.evil.com` 400 sin `Access-Control-Allow-Origin`
- [x] 9.5 Rate limiting en `/auth/login` (AC: 6 intentos malos seguidos → 429 al sexto) — 429 real al sexto directo a `:8000` y vía `:8080/api`; rotar `X-Real-IP`/`X-Forwarded-For` no lo evade; otro cliente detrás de nginx no comparte bucket
- [x] 9.6 `/connections/telegram/confirm` valida `X-N8N-SECRET` (AC: sin header → 401; probado en 4.3) — sin header, vacío, malo y no ASCII dan 401 (el no ASCII daba 500: corregido); comparación en tiempo constante
- [x] 9.7 Nada de credenciales/tokens/claves en código ni en `docker-compose.yml`; `.env` en `.gitignore`; `.env.example` con placeholders (AC: `git grep` de tokens/claves no encuentra valores reales; `git status` muestra `.env` sin trackear) — `git grep` sobre HEAD y auditoría de los 105 commits de todas las refs (incluye `main..HEAD`) sin coincidencias reales; `.env` ignorado y nunca commiteado
- [ ] 9.8 Aislamiento entre cuentas: todas las queries filtran por `client_account_id` del token (AC: dos cuentas seed no se ven datos entre sí en ningún endpoint autenticado) — **sin tildar (cumple con salvedad)**: perfil, conexiones, códigos de Telegram, estado de Gmail y WhatsApp nunca cruzan cuentas (tests + curl), pero `orders`, `tickets`, `products` y las métricas son globales por diseño (un comercio por instalación, sin `client_account_id` y sin poder modificarlas): todas las cuentas ven lo mismo. Ver `SPEC_DASHBOARD_CLIENTE.md` §10.2 y `DASHBOARD_ALLOW_REGISTRATION`